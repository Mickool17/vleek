...
def validate_email(email: str) -> Tuple[bool, str]:
    ...
    if any(not part for part in domain_parts):
        return False, "Email domain cannot have consecutive dots"
    
    # Check domain length
    if len(domain) > 253:
        return False, "Email domain is too long"
    
    # Final regex check for proper format
...
def validate_phone(phone: str) -> Tuple[bool, str]:
    """Comprehensive phone validation with helpful error messages"""
    if not phone:
    ...
    # Check for US phone number patterns
    us_patterns = [
    ...
    return False, "Please enter a valid phone number (e.g., (555) 123-4567 or 555-123-4567)"

def validate_address(address: str) -> Tuple[bool, str]:
    """Basic address validation with helpful error messages"""
    if not address:
    ...
    if len(address) < 10:
        return False, "Address is too short (please include street number and name)"
    
    if len(address) > 200:
        return False, "Address is too long"
    
    # Check for basic components (at least one number and some letters)
    ...
    if not has_letters:
        return False, "Address should include street name"
    
    # Check for obvious incomplete addresses
    if address.lower().strip() in ['address', 'street', 'home', 'house']:
        return False, "Please enter your complete address"
    
    return True, "Valid address"

def validate_name(name: str) -> Tuple[bool, str]:
    """Name validation with helpful error messages"""
    if not name:
...
- If multiple quantities mentioned for different items, parse each separately"""
    ...
    def handle_intent(self, intent: str, user_input: str, session_id: str, confidence: float) -> Dict:
        ...
        elif current_step == 'removing_item':
            return self.handle_item_removal(user_input, session_id)
    ...
    def handle_info_collection(self, user_input: str, session_id: str) -> Dict:
        ...
        if 'name' not in customer_info:
            ...
            is_valid, error_message = validate_name(user_input)
            if is_valid:
    ...
    def handle_logistics_info_collection(self, user_input: str, session_id: str) -> Dict:
        ...
        if 'full_name' not in logistics_info:
            ...
            if is_valid:
                ...
                return {
                    'message': f"📅 **Pickup Date (e.g., Monday, {(datetime.now() + timedelta(days=7)).strftime('%b %d')} or {(datetime.now() + timedelta(days=7)).strftime('%m/%d/%Y')}):**",
                    'type': 'logistics_info_collection',
            ...
            if parsed_date:
                ...
                return {
                    'message': f"Please enter a valid date (e.g., Tomorrow, Monday, {(datetime.now() + timedelta(days=7)).strftime('%m/%d/%Y')}, or {(datetime.now() + timedelta(days=7)).strftime('%b %d')}):",
                    'type': 'logistics_info_collection',
                    'collecting': 'pickup_date'
                }
...
Please type **"confirm"** to proceed with your logistics service request."""
    ...
    def handle_view_cart(self, session_id: str) -> Dict:
        """Handle view cart request with detailed item information"""
        session = self.customer_sessions[session_id]
        cart = session.get('cart', [])
        ...
        for i, item in enumerate(cart, 1):
            ...
            if item['options']:
                message += f"  • Options: {', '.join(item['options'])}\n"
            message += f"  • Price: ${item_total:.2f}\n\n"
    ...
    def handle_remove_item_request(self, session_id: str) -> Dict:
        ...
        # Show cart with item numbers for removal
        message = "🛒 **Select an item to remove:**\n\n"
        
        for i, item in enumerate(cart, 1):
            message += f"**{i}.** {item['quantity']}x {item['name']}"
            if item['options']:
                message += f" ({', '.join(item['options'])})"
            message += f" - ${item['total']:.2f}\n"
        
        message += "\nPlease type the **item number** you want to remove (e.g., \"1\", \"2\", etc.)"
        
        # Set the current step to handle the next input
        session['current_step'] = 'removing_item'
        self.customer_sessions[session_id] = session
        
        return {
            'message': message,
            'type': 'item_removal',
            'suggestions': [str(i) for i in range(1, len(cart) + 1)] + ["Cancel"]
        }
    
    def handle_item_removal(self, user_input: str, session_id: str) -> Dict:
        """Handle actual item removal by number"""
        session = self.customer_sessions[session_id]
        cart = session.get('cart', [])
        ...
        if user_input == 'cancel':
            ...
            self.customer_sessions[session_id] = session
            return {
                'message': "❌ Item removal cancelled.\n\nWhat would you like to do next?",
                'type': 'cart_update',
                'suggestions': [
                    "View Full Cart",
                    "Add More Items", 
        ...
        try:
            ...
            if 1 <= item_number <= len(cart):
                ...
                message = f"✅ **Removed from cart:** {removed_item['quantity']}x {removed_item['name']}"
                if removed_item['options']:
                    message += f" ({', '.join(removed_item['options'])})"
                ...
                return {
                    'message': f"❌ Invalid item number. Please enter a number between 1 and {len(cart)}, or 'Cancel'.",
                    'type': 'item_removal',
                    'suggestions': [str(i) for i in range(1, len(cart) + 1)] + ["Cancel"]
                }
        except ValueError:
            return {
                'message': "❌ Please enter a valid item number (e.g., '1', '2') or 'Cancel'.",
                'type': 'item_removal',
                'suggestions': [str(i) for i in range(1, len(cart) + 1)] + ["Cancel"]
            }
    ...
    def handle_clear_cart(self, session_id: str) -> Dict:
        ...
        return {
            'message': "🗑️ **Cart cleared!** All items have been removed.\n\nWould you like to start a new order?",
            'type': 'cart_cleared',
            'suggestions': [
                "📋 Place an Order",
                "🧼 Browse Services",
    ...
    def handle_checkout(self, session_id: str) -> Dict:
        ...
        if not pickup_info.get('pickup_date'):
            ...
            # Generate dynamic date suggestions
            today = datetime.now()
            tomorrow = today + timedelta(days=1)
            next_week = today + timedelta(days=7)
            
            return {
                'message': f"📅 **Pickup Scheduling**\n\nGreat! Let's schedule your pickup.\n\n**Please provide your pickup date** (e.g., {tomorrow.strftime('%Y-%m-%d')}, Tomorrow, Monday):",
                'type': 'pickup_scheduling',
                'suggestions': [
                    "Tomorrow",
                    "Monday", 
    ...
    def handle_session_resumption_choice(self, user_input: str, session_id: str) -> Dict:
        ...
        if 'continue' in user_input_lower or 'left off' in user_input_lower:
            ...
            if last_step == 'selecting_items':
                ...
                return {
                    'message': f"Great! Let's continue with your pickup scheduling.\n\n📅 **When would you like us to pick up your items?** (e.g., Tomorrow, Monday, {(datetime.now() + timedelta(days=7)).strftime('%m/%d/%Y')})",
                    'type': 'pickup_scheduling',
                    'collecting': 'pickup_date'
                }
            elif last_step == 'collecting_info':
